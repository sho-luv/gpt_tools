...
class Credentials:
    ...
    def parse(
        ...
        return cls(domain, username, password, lm_hash, nt_hash, aes_key, use_kerberos)
...
class DirectorySession:
    ...
    def connect(
        ...
        if credentials.kerberos:
            target = dc_host or _get_machine_name(dc_ip or query_domain, True)
        else:
            target = dc_host or dc_ip or query_domain
